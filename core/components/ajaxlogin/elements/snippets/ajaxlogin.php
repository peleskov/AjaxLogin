...
switch ($service){
    ...
    case 'login':
        /*
            Необходимо закоментировать строку в ../core/components/login/controllers/web/Login.php
            //$this->modx->sendRedirect($this->modx->getOption('site_url'));
            что бы в логах не было ошибки при успешной авторизации
        */
        $props = array(
            'loginTpl' => 'tpl.signin.modal.form',
            'actionKey' => 'service',
            'loginKey' => 'login',
            'logoutKey' => 'logout',
        ...
        $modx->runSnippet('Login', $props);
        foreach($modx->placeholders as $key => $ph){if(strpos($key, 'errors') === 0) $placeholders[$key] = $ph;}
        if($placeholders['errors']) return $AjaxForm->error($placeholders['errors'], array('service' => $service));
        else return $AjaxForm->success('Авторизован', array('service' => $service, 'location' => $modx->makeUrl(15)));
        break;
    case 'forgotpass':
    ...
    case 'changepass':
        ...
        if( $placeholders[$scriptProperties['placeholderPrefix'].'successMessage'] != '1'){//ERROR
            foreach($placeholders as $key => $ph){if(strpos($key, $scriptProperties['placeholderPrefix'].'error.') === 0) $errors[str_replace($scriptProperties['placeholderPrefix'].'error.', '', $key)] = $ph;}
    ...
    case 'updateprof':
        ...
        if(!$placeholders['login.update_success']){//ERROR
            ...
            foreach($placeholders as $key => $ph){if(in_array(str_replace($scriptProperties['placeholderPrefix'], '', $key), $fields)) $values[str_replace($scriptProperties['placeholderPrefix'], '', $key)] = $ph;}
